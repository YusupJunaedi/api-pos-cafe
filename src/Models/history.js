...
const historysModel = {
  getAllHistorys: () => {
    const queryString =
      "SELECT invoice.no_invoice, invoice.cashir, invoice.amount, product.name_product ,history.qty, history.total_price FROM invoice JOIN history ON history.invoice=invoice.no_invoice JOIN product ON product.id_product = history.product_id";
